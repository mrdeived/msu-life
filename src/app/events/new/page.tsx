...
async function createEvent(formData: FormData) {
  ...
  // Admin guard for server action
  const { prisma: actionDb } = await import("@/lib/prisma");
  const ADMIN_EMAILS_SA = (process.env.ADMIN_EMAILS ?? "")
    .split(",")
    .map((e: string) => e.trim().toLowerCase())
    .filter(Boolean);
  const actionUser = await actionDb.user.findUnique({
    where: { id: user.id },
    select: { id: true, email: true, isAdmin: true, isActive: true, isBanned: true },
  });
  if (
    !actionUser || !actionUser.isActive || actionUser.isBanned ||
    (!actionUser.isAdmin && !ADMIN_EMAILS_SA.includes(actionUser.email.toLowerCase()))
  ) {
    redirectServer("/home");
  }

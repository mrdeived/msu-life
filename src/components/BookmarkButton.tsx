...
export default function BookmarkButton({ eventId, initialBookmarked }: { eventId: string; initialBookmarked: boolean }) {
  ...
  async function toggle() {
    ...
    await fetch(`/api/events/${eventId}/bookmark`, {
      method: initialBookmarked ? "DELETE" : "POST",
    });
    router.refresh();
    setLoading(false);
  }
  ...
  return (
    ...
    >
      {loading ? "…" : initialBookmarked ? "Bookmarked" : "Bookmark"}
    </button>
  );
}
